...
rs = backend$response_to_backend(app$process_request(request_json))
expect_equal(rs[[1]], "{\"answer\":\"json\"}")

request_text = Request$new(path = "/text")
rs = backend$response_to_backend(app$process_request(request_text))
expect_equal(rs[[1]], "text")

request_uct = Request$new(path = "/unknown-content-type")
rs = backend$response_to_backend(app$process_request(request_uct))
expect_true(is.character(rs[[1]]))

request_rds = Request$new(path = "/rds")
rs = backend$response_to_backend(app$process_request(request_rds))
rs = unserialize(rs[[1]])
expect_equal(rs, list(answer = "rds"))

request_rds2 = Request$new(path = "/rds2")
rs = backend$response_to_backend(app$process_request(request_rds2))
rs = unserialize(rs[[1]])
expect_equal(rs, list(answer = "rds2"))

HTTPError$set_content_type("application/json")
request_404 = Request$new(path = "/404")
rs = backend$response_to_backend(app$process_request(request_404))
...
rs = backend$response_to_backend(app$process_request(rq))
