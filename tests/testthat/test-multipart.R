...
test_that("Test parse_multipart_body", {
  ...
  expect_is(p, "list")
  expect_equal(p$files$file1$filename, "test1")
  expect_equal(p$files$file1$content_type, "application/octet-stream")
  expect_equal(p$files$file1$offset, 252L)
  expect_equal(p$files$file1$length, 5L)
  expect_equal(p$values$param1, "value1")
  f1c = rawToChar(r[seq(p$files$file1$offset, by = 1, length.out = p$files$file1$length)])
  expect_equal(f1, f1c)
})
