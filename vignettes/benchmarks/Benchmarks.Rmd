...
run_apib = function(n_threads = c(1, 2, 4, 6), n_sec = 5, keep_alive = 10000, runtime_checks = "FALSE") {
  results = character()
  for (n_threads in n_threads) {
      res = system2(
        command = "apib", 
        args = sprintf("-c %d -d %d -k %d --csv-output \'http://127.0.0.1:8080/fib?n=10\'", n_threads, n_sec, keep_alive),
        stdout = TRUE
      )
...
apib_bench = function(n_sec, content_type, keep_alive = 10000) {
  results = character()
  for (runtime_checks in c("FALSE", "TRUE")) {
...
```{r, echo=FALSE}
plot_results(results_no_keep_alive, "application/json", adj_y_text = 2)
```
